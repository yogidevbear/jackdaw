...
(defn with-topic-producer
  [{:keys [producer-config topic-config]} f]
  (let [prefix (test-prefix)
        topic-config (update topic-config
                             :topic-name #(str prefix "-" %))
        topic-fix (fix/topic-fixture (broker-config) {prefix topic-config}
                                     {:timeout-ms 1000
                                      :delete-first? true})]
    (fix/with-fixtures [topic-fix]
      (try
        (with-open [producer (client/producer producer-config)]
          (f topic-config producer))
        (finally
          (with-open [admin (admin/->AdminClient (broker-config))]
            (admin/delete-topics! admin [topic-config])))))))

(deftest ^:integration send!-test
  (with-topic-producer {:producer-config (producer-config)
                        :topic-config foo-topic}
    (fn [topic producer]
      (testing "simple send"
        (let [msg (data/->ProducerRecord topic "1" "one")
...
(defn with-topic-consumer
  [{:keys [consumer-config topic-config]} f]
  (let [prefix (test-prefix)
        topic-config (update topic-config
                             :topic-name #(str prefix "-" %))
        topic-fix (fix/topic-fixture (broker-config) {prefix topic-config}
    ...
    (fix/with-fixtures [topic-fix]
      (try
        (with-open [consumer (client/consumer consumer-config)]
          (f topic-config consumer))
        (finally
          (with-open [admin (admin/->AdminClient (broker-config))]
            (admin/delete-topics! admin [topic-config])))))))
...
(deftest ^:integration consumer-test
  (let [config {"bootstrap.servers" "localhost:9092"
    ...
    (testing "create with config"
      (with-open [consumer (client/consumer config)]
        (is (instance? Consumer consumer))))


    (testing "create with config and serdes"
      (with-open [consumer (client/consumer config {:key-serde key-serde
                                                    :value-serde value-serde})]
        (is (instance? Consumer consumer))))

    (testing "topic subscription"
      (with-topic-consumer {:consumer-config config
                            :topic-config foo-topic}
        (fn [topic consumer]
    ...
    (testing "topic assignment"
      (with-topic-consumer {:consumer-config config
        ...
        (fn [topic consumer]
          (let [consumer (-> consumer
                             (client/assign-all [(:topic-name topic)]))]
            (is (instance? Consumer consumer))
            (is (= [(-> (select-keys topic [:topic-name])
                          (assoc :partition 0))]
                     (client/assignment consumer)))))))))

(deftest ^:integration partitions-for-test
  (testing "partition info"
    (with-topic-consumer {:consumer-config (consumer-config "partition-test")
                          :topic-config bar-topic}
      (fn [topic consumer]
        (let [[pinfo] (-> consumer
                          (client/subscribe [topic])
                          (client/partitions-for topic)
                          (data/datafy))]
  ...
  (testing "single partition consumer"
    (with-topic-consumer {:consumer-config (consumer-config "partition-test")
      ...
      (fn [topic consumer]
        (let [consumer (-> consumer
                           (client/subscribe [topic]))]
          (is (= 1 (client/num-partitions consumer topic)))))))

  (testing "multi partition consumer"
    (with-topic-consumer {:consumer-config (consumer-config "partition-test")
                          :topic-config high-partition-topic}
      (fn [topic consumer]
        (let [consumer (-> consumer
                           (client/subscribe [topic]))]
          (is (= 15 (client/num-partitions consumer topic))))))))
...
(deftest ^:integration position-all-test
  (with-topic-consumer {:consumer-config (consumer-config "position-all-test")
                        :topic-config bar-topic}
    (fn [topic consumer]
      (let [consumer (-> consumer
